...
def build_transform(is_train, args):
    mean = tuple(args.mean.tolist())
    std = tuple(args.std.tolist())
    # train transform
    if is_train:
    ...
    #t.append(transforms.Resize(size, interpolation=PIL.Image.BICUBIC),)  # to maintain same ratio w.r.t. 224 images
